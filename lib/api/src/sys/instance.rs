...
impl Instance {
    ...
    /// TODO add docs
    pub fn new_by_index(module: &Module, externs: Vec<Extern>) -> Result<Self, InstantiationError> {
        let store = module.store();
        let imports = externs.iter().map(Extern::to_export).collect::<Vec<_>>();
        let handle = module.instantiate(imports.clone())?;
        let exports = module
            .exports()
            .map(|export| {
                let name = export.name().to_string();
